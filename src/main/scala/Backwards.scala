 ...
 * NOTE: not able to find a solution for scala that satisfies the implementation
 */
  ...
  def bind[A, B](m: S[A], f: A => S[B]): S[B] = {
    (s2) => {
      class T {
        ...
        class LazyA(_that: => (A, State)) {
          lazy val that = _that
        }

        class LazyB(_that: => (B, State)) {
          lazy val that = _that
        }

        // mutually recursive expression lazy evaluation (not the same as haskell, doesn't work)
        //      lazy val (a: A, s0: State) = m(s1)
        //      lazy val (b: B, s1: State) = f(a)(s2)
        //      (b,s0)
        lazy val a: LazyA = new LazyA({m(b.that._2)})
        lazy val b: LazyB = new LazyB({f(a.that._1)(s2)})
        lazy val r: (B, State) = (b.that._1, a.that._2)
      }
